  ...
  const onsubmit = async (e) => {
    ...
    if (number < 1 || number > 9) {
      setdata({
        ...formdata,
        number: number,
        valuerror: "you need to enter a number between 1-9",
      });
    } else {
      const config = {
        headers: {
  ...
  if (formdata.question === "Sucsses") {
    ...
    return (
      <div className='verify'>
        <div className='text'>{nl2br(text)}</div>
        <Table className='clientboard'>
          ...
          <tbody>
            {formdata.clientboard.map((row) => (
              <tr id={`row-${formdata.clientboard.indexOf(row)}`}>
                <strong>{formdata.clientboard.indexOf(row)}</strong>
                {row.map((col) => (
                  <td id={`col-${row.indexOf(col)}`}>{col}</td>
                ))}
              </tr>
            ))}
          </tbody>
        ...
        <Table className='serverboard'>
          ...
          <tbody>
            {formdata.serverboard.map((row) => (
              <tr id={`row-${formdata.serverboard.indexOf(row)}`}>
                <strong>{formdata.serverboard.indexOf(row)}</strong>
                {row.map((col) =>
                  col !== 0 ? (
                    <td id={`col-${row.indexOf(col)}`}>{col}</td>
                  ) : (
                    <td id={`col-${row.indexOf(col)}`}>{""}</td>
                  )
                )}
              </tr>
            ))}
